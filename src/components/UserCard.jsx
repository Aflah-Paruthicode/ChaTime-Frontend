    ...
    <div className="card w-80 h-fit bg-white border border-white/10 shadow-2xl overflow-hidden hover:border-blue-500/50 transition-all duration-300 group">
      <figure className="relative h-64 overflow-hidden">
        ...
        <div className="absolute top-4 right-4">
          <span className="badge badge-primary px-3 py-1 font-semibold text-xs uppercase tracking-wider">{user.gender}</span>
        </div>
      </figure>
      ...
      <div className="card-body p-6">
        <div className="space-y-1">
          ...
          <p className="text-sm text-gray-500 line-clamp-2 italic leading-relaxed">"{user.about}"</p>
        </div>

        <div className="mt-4 flex flex-wrap gap-2">
          {user.skills.map((skill) => (
            <span key={skill} className="text-[10px] font-bold px-2 py-1 rounded-md bg-white/5 border border-gray-600 text-gray-600 uppercase tracking-tighter">
              {skill}
            </span>
          ))}
        </div>
